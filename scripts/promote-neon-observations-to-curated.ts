...
const BATCH_SIZE = Number(process.env.CURATION_PROMOTE_BATCH_SIZE || 100);
...
async function main() {
  const sourceRows = await withNeonClient(async (client) =>
    client.query(
      `
      SELECT
        observation_id,
  ...
  let promoted = 0;
  for (const rows of chunkArray(sourceRows.rows, BATCH_SIZE)) {
  ...
  console.log(JSON.stringify({ promoted, minQuality: MIN_QUALITY, batchSize: BATCH_SIZE }, null, 2));
}
